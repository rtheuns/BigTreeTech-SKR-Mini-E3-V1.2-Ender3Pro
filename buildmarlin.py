...
PINS = f'{MARLIN_DIR}/Marlin/src/pins/stm32f1/pins_BTT_SKR_MINI_E3_common.h'
...
def build_codebase():
    ...
    shutil.copy(f'{MARLIN_DIR}/.pio/build/STM32F103RC_btt/firmware.bin', './Build/firmware.bin')
...
#
def set_environment():
    sed(r'default_envs = .*', 'default_envs = STM32F103RC_btt', PLATFORMIO_INI, '#')
...
def set_home_and_parking():
  ...
  sed(r'#define HOMING_FEEDRATE_MM_M.*', '#define HOMING_FEEDRATE_MM_M { (60*60), (60*60), (20*60) }', CONFIGURATION_H)
  ...
  sed(r'.*#define EVENT_GCODE_SD_ABORT.*', '  #define EVENT_GCODE_SD_ABORT "G27 P2"       // G-code to run on Stop Print (e.g., "G28XY" or "G27")', CONFIGURATION_ADV_H)
  ...
  sed(r'.*#define FILAMENT_LOAD_UNLOAD_GCODES.*', '  #define FILAMENT_LOAD_UNLOAD_GCODES           // Add M701/M702 Load/Unload G-codes, plus Load/Unload in the LCD Prepare menu.', CONFIGURATION_ADV_H)
...
def set_bltouch():
  ...
  sed(r'.*#define PROBING_MARGIN .*', '#define PROBING_MARGIN 31', CONFIGURATION_H) # 44 or 31

  sed(r'.*#define XY_PROBE_FEEDRATE .*', '#define XY_PROBE_FEEDRATE (133*60)', CONFIGURATION_H)
  sed(r'.*#define Z_PROBE_FEEDRATE_FAST .*', '#define Z_PROBE_FEEDRATE_FAST (10*60)', CONFIGURATION_H)
